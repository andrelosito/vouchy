...
//Insert the Records in the Bank Orders Table
function addRec(doc) {
    ...
    <td>${doc.data().category}</td>  
      ...
      <td>${doc.data().bankfullName}</td>
      <td>${doc.data().bankemail}</td>
...
//Edit Trigger Function in the Bank Orders Table
function editRec(doc) {
    $('.edit').click((e) => {
        e.stopImmediatePropagation();
        var id = e.target.id;

        db.collection('bank orders').doc(id).get().then(doc => {
            ...
            $('#transactionId').val(doc.data().transactionId);
            $('#bankfullName').val(doc.data().bankfullName);
            $('#bankemail').val(doc.data().bankemail);
...
//Delete the Records in the Bank Orders Collection in Firestore
function delRec(doc) {
    $('.delete').click((e) => {
        e.stopImmediatePropagation();
        var id = e.target.id;

        db.collection('bank orders').doc(id).delete();

        swal("Oh no!", "A data was deleted!", "warning");
    })
}

//Add Records in the Bank Orders Collection in Firestore
dataform.on('submit', (e) => {
    ...
    swal("Success!", "An ordered was created!", "success");

})

//Update Records in the Bank Orders Collection in Firestore
$('#update').on('click', () => {
    var id = $('#document').val();

    db.collection('bank orders').doc(id).set({
        brand: $('#brand').val(),
        price: $('#price').val(),
        location: $('#location').val(),
        description: $('#description').val(),
...
//Other Functions
db.collection('bank orders').onSnapshot(snapshot => {
...
function generate() {
    ...
    var y = 30;
    doc.setLineWidth(2);
    doc.text(220, y = y + 30, "                    BANK ORDERS TABLE");
...
$('#export').click(function() {
    ...
    downloadLink.download = "BANK ORDERS.csv";
