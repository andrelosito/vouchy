...
//Insert the Records in the Credit Orders Table
function addRec(doc){
...
//Delete the Records in the Credit Orders Collection in Firestore 
function delRec(doc){
  $('.delete').click((e)=>{
    e.stopImmediatePropagation();
    var id=e.target.id;

    db.collection('credit orders').doc(id).delete();

    swal("Oh no!", "A data was deleted!", "warning");
  })
}

//Edit Trigger Function in the Credit Orders Table
function editRec(doc){
  $('.edit').click((e)=>{
    e.stopImmediatePropagation();
    var id=e.target.id;

    db.collection('credit orders').doc(id).get().then(doc=>{
        ...
        $('#cardNumber').val(doc.data().cardNumber);
        $('#mm').val(doc.data().mm);
        $('#yy').val(doc.data().yy);
        $('#cvv').val(doc.data().cvv);
...
//Add Records in the Credit Orders Collection in Firestore
dataform.on('submit',(e)=> {
  ...
  db.collection('credit orders').add({
   ...
   swal("Success!", "An order was created", "success");

})

//Update Records in the Credit Orders Collection in Firestore
$('#update').on('click',()=>{
  var id = $('#document').val();

  db.collection('credit orders').doc(id).set({
      brand:$('#brand').val(),
      price:$('#price').val(),
     location:$('#location').val(),
      description:$('#description').val(),
...
//Other Functions
db.collection('credit orders').onSnapshot(snapshot=>{
...
//Export as pdf
  function generate() {  
      ...
      var y = 30;  
      doc.setLineWidth(2);  
      doc.text(220, y = y + 30, "                CREDIT ORDERS TABLE");
  ...
  //export as csv
    $('#export').click(function() {
        ...
        downloadLink.download = "CREDIT ORDERS.csv";
